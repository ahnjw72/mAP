...
print("cd to {}".format(DR_PATH))
os.chdir(DR_PATH)

#SEPARATOR_KEY = 'Enter Image Path:'
SEPARATOR_KEY = 'Predicted in'
EXCLUDE_KEY1 = 'Enter Image Path:'
EXCLUDE_KEY2 = 'Detection layer:'
IMG_FORMAT = '.jpg'
...
with open(IN_FILE) as infile:
    for line in infile:
        #print(line)
        if EXCLUDE_KEY1 in line:
            continue

        if EXCLUDE_KEY2 in line:
            continue
        ...
        if SEPARATOR_KEY in line:
            ...
            image_name = tmp_str.split('/')[-1].split('.')[0]
            print(image_name)
